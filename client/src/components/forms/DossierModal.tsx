...
const DossierModal: React.FC<DossierModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      if (dossier) {
        ...
        // Trouver le BAP complet dans la liste
        const fullBap = baps.find(bap => bap.id === dossier.bap?.id) || null
        setSelectedBap(fullBap)
      } else {
  ...
  }, [isOpen, dossier, reset, sgamis, users, baps, currentUser])
                ...
                <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      >
                        <div className="relative">
                          <Listbox.Button className="relative w-full cursor-default rounded-lg bg-white py-3 pl-3 pr-10 text-left shadow-sm border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                            tabIndex={0}>
                            <span className="block truncate">
                              {selectedBap ? selectedBap.nomBAP : 'Sélectionner un BAP'}
                            </span>
                            <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
                              <ChevronUpDownIcon
                                className="h-5 w-5 text-gray-400"
                      ...
                      {errors.bapId && (
                        <p className="mt-1 text-sm text-red-600">{errors.bapId.message}</p>
                      )}
                    </div>
