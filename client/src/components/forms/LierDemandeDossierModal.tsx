...
const LierDemandeDossierModal: React.FC<LierDemandeDossierModalProps> = ({
  isOpen,
  onClose,
  demandeId,
  demandeNumeroDS
  ...
  const lierDemandeMutation = useMutation({
    mutationFn: async (dossierId: string) => {
      const response = await api.put(`/demandes/${demandeId}`, {
        dossierId
      })
      return response.data
    },
    onSuccess: () => {
      ...
      toast.success('Demande liée au dossier avec succès')
      onClose()
    },
    onError: (error: any) => {
  ...
  const handleLierDemande = (dossierId: string) => {
    lierDemandeMutation.mutate(dossierId)
  }
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">
              Lier la demande à un dossier
            </h2>
            <p className="text-sm text-gray-600 mt-1">
              Demande n°{demandeNumeroDS}
            </p>
          </div>
          <button
            onClick={onClose}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredDossiers.map((dossier: Dossier) => (
                  <tr key={dossier.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        onClick={() => handleLierDemande(dossier.id)}
                        disabled={lierDemandeMutation.isPending}
...
export default LierDemandeDossierModal