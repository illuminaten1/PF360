...
interface DepenseOrdonneeSgami {
  libelle: string
  nombre: number
  pourcentage?: number
...
interface DepensesOrdonneesParSgamiData {
  statistiques: DepenseOrdonneeSgami[]
  budgetTotal?: number
}

interface DepensesOrdonneesParSgamiPanelProps {
  statsDepensesOrdonneesParSgami: DepensesOrdonneesParSgamiData | undefined
}

const DepensesOrdonneesParSgamiPanel: React.FC<DepensesOrdonneesParSgamiPanelProps> = ({ statsDepensesOrdonneesParSgami }) => {
  const heightPercentage = statsDepensesOrdonneesParSgami && statsDepensesOrdonneesParSgami.statistiques.length > 0 ? (100 / statsDepensesOrdonneesParSgami.statistiques.length).toFixed(2) : '100'
  ...
  const formatValue = (stat: DepenseOrdonneeSgami) => {
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full h-full border-collapse" style={{ tableLayout: 'fixed' }}>
          ...
          <tbody className="bg-white h-full">
            {statsDepensesOrdonneesParSgami && statsDepensesOrdonneesParSgami.statistiques.length > 0 ? (
              statsDepensesOrdonneesParSgami.statistiques.map((stat, index) => {
...
export default DepensesOrdonneesParSgamiPanel