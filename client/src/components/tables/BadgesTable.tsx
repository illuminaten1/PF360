...
const BadgesTable: React.FC<BadgesTableProps> = ({
  ...
  const getColorPreview = (couleur?: string) => {
    ...
    return (
      <div className="flex items-center space-x-2">
        <div 
          ...
          title={couleur}
        />
        <span className="text-xs text-gray-500 font-mono">{couleur}</span>
      </div>
    )
  }

  const columns = useMemo<ColumnDef<Badge>[]>(
    () => [
      {
        accessorKey: 'nom',
        header: 'Nom du badge',
      ...
      },
      {
        ...
        header: 'Couleur',
        cell: ({ getValue }) => {
          const couleur = getValue<string>()
          return couleur ? getColorPreview(couleur) : (
            <span className="text-sm text-gray-400">Aucune couleur</span>
          )
        },
        enableColumnFilter: true,
        filterFn: 'includesString'
