...
interface DecisionsTableV2Props {
  data: Decision[]
  loading?: boolean
...
const DecisionsTableV2: React.FC<DecisionsTableV2Props> = ({
  ...
  // État pour les décisions avec demandeurs expandés
  ...
  const columns = useMemo<ColumnDef<Decision>[]>(
      ...
      },
      {
        ...
        meta: {
          filterComponent: (column: any) => (
            <MultiSelectFilter
              column={column}
              options={facets?.types || ['AJ', 'AJE', 'PJ', 'REJET']}
      ...
      },
      {
        ...
        meta: {
          filterComponent: (column: any) => (
            <MultiSelectFilter
              column={column}
              options={facets?.typeVictMecs || ['VICTIME', 'MIS_EN_CAUSE']}
              placeholder="Tous"
            />
          )
        }
          ...
          return (
            <div className="flex items-center text-sm">
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation()
                  handleDateClick(decision, 'dateSignature')
                }}
          ...
          return (
            <div className="flex items-center text-sm">
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation()
                  handleDateClick(decision, 'dateEnvoi')
                }}
                className="text-gray-900 hover:text-blue-600 hover:underline transition-colors"
              >
                {date ? dayjs(date).format('DD/MM/YYYY') : 'Non définie'}
    ...
    [onView, onEdit, onDelete, facets, handleViewDossier, expandedDecisions]
  )
          ...
          <div className="hidden sm:flex items-center gap-3">
            ...
            {onClearFilters && (columnFilters.length > 0 || globalFilter) && (
              <button
                ...
                title="Effacer tous les filtres et afficher toutes les décisions"
              >
...
export default DecisionsTableV2
