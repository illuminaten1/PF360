...
const PCETable: React.FC<PCETableProps> = ({
  data,
  loading = false,
  onEdit,
  onDelete
  ...
  const columns = useMemo<ColumnDef<PCE>[]>(
    () => [
      ...
      {
        accessorKey: 'pceNumerique',
        header: 'PCE numérique',
        cell: ({ getValue }) => {
          const pceNumerique = getValue<string>()
          return (
      ...
      {
        accessorKey: 'codeMarchandise',
        header: 'Code marchandise',
        cell: ({ getValue }) => {
          const codeMarchandise = getValue<string>()
          return (
