...
interface RevueConventionsTableProps {
...
const RevueConventionsTable: React.FC<RevueConventionsTableProps> = ({ 
  selectedUserId, 
  selectedUser 
}) => {
  ...
  const fetchDemandesSansConvention = async () => {
    ...
    try {
      ...
      const demandesAvecDecisionPjSansConvention = response.data.demandes.filter((demande: Demande) => {
        ...
        const hasConvention = demande.conventions && demande.conventions.length > 0
        
        return hasDecisionPj && !hasConvention
      })
      
      setData(demandesAvecDecisionPjSansConvention)
    } catch (error) {
      console.error('Error fetching demandes avec décision PJ sans convention:', error)
      setData([])
    } finally {
      setLoading(false)
    }
  ...
  const handleSaveComment = async () => {
    ...
    try {
      ...
      await api.put(`/demandes/${editingCell.demandeId}`, {
        commentaireConvention: editingCell.value
      })

      setData(prevData => 
        prevData.map(demande => 
          demande.id === editingCell.demandeId 
            ? { ...demande, commentaireConvention: editingCell.value }
            : demande
        )
      )
      ...
      <div className="p-8 text-center">
        <div className="flex flex-col items-center">
          ...
          <p className="text-sm text-gray-500">
            Veuillez sélectionner un utilisateur pour voir ses demandes avec décision PJ sans convention.
          </p>
        </div>
      </div>
    )
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-medium text-gray-900">
                Demandes avec décision PJ sans convention - {selectedUser.grade && `${selectedUser.grade} `}{selectedUser.prenom} {selectedUser.nom}
              </h3>
              <p className="mt-1 text-sm text-gray-600">
                {data.length} demande{data.length !== 1 ? 's' : ''} avec décision PJ sans convention
              </p>
            </div>
            {data.length > 0 && (
              <button
                    ...
                    <div className="flex flex-col space-y-2 min-h-[80px]">
                      ...
                      {header.column.getCanFilter() && (
                        <div>
                          ...
                          ) : header.column.id === 'dateDecisionPj' ? (
              ...
              <tr>
                ...
                >
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-sm font-medium text-gray-900 mb-1">Aucune demande avec décision PJ sans convention</p>
                    <p className="text-sm text-gray-500">Toutes les demandes avec décision PJ ont une convention ou aucune décision PJ</p>
                  </div>
                </td>
              </tr>
            ) : (
...
export default RevueConventionsTable